...
class ClockWidgetProvider : AppWidgetProvider() {
    ...
    companion object {
        // Наше собственное действие для обновления
        const val ACTION_UPDATE_WIDGET = "ysg.home.cstmclckwdgt.action.UPDATE_WIDGET"
    }

    // --- ГЛАВНОЕ ИЗМЕНЕНИЕ ---
    // Этот метод теперь будет ловить ВСЕ сигналы, приходящие провайдеру
    override fun onReceive(context: Context, intent: Intent) {
        // Сначала вызываем родительский метод, это важно!
        super.onReceive(context, intent)

        // А теперь проверяем, не наш ли это сигнал пришел от TimeTickReceiver
